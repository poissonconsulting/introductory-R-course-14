...
author: "Dr. Joseph L. Thorley R.P.Bio."
date: "August 24^th^, 2014"
output:
  html_document:
    keep_md: yes
    toc: yes
...
They are available [online](http://www.poissonconsulting.ca/temporary-hidden-link-3423423234/courses/introductory-R.html) and as a [pdf](introductory-R.pdf).
...
```{r, echo = FALSE, results = 'asis'}
exercise("Run the line of code above and then open the StocksMissing csv file in a spreadsheet program and replace the first five values in the `X` column with `NA`. Now use the `read.csv` function to import the ToothGrowthMissing csv file into the workspace. Use the `na.omit` function to delete the 
...
ToothGrowth <- select(datasets::ToothGrowth, len2, supp, dose)
ToothGrowth <- arrange(datasets::ToothGrowth, dose, supp, len2)
```
...
To improve readability, the same operations can be joined in left to right order using the forward-pipe operator `%>%` in the `magrittr` package
to 
```{r, eval = FALSE}
...
The following code uses the `tidyr::gather` function to convert the `stocks` data frame from wide to long format so that it now satisfies the first condition. 
```{r}
install.packages("tidyr")
library(tidyr)
stocks <- data.frame( time = 1:10, X = seq(-2,3, length.out = 10),
  Y = 3:12, Z = -10:-1)
...
```{r}
head(spread(stocks, stock, price))
```

```{r, echo = FALSE, results = 'asis'}
exercise("What happens if you filter negative prices from the gathered `stocks` data frame before spreading?")
```
...
simply defines the function. List the contents of your workspace  using `ls()` and you will see
...
To call the function you have to remember to follow 
