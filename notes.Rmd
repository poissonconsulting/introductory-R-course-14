...
You can load a package by selecting it or by typing `library(package_name)` at the console.
...
You can install a package from the CRAN site onto your computer by clicking Install on the Packages window and entering its name alternatively type `install.packages("package_name")`.
```{r, echo = FALSE, results = 'asis'}
exercise("Install the package `ggplot2` onto your computer and then load it. What arguments does the function `ggplot` take.")
```

To install a package from a GitHub repository you need to first load the `devtools` package and then use the `install_github` function. 
...
exercise("Install the latest version of the `jaggernaut` package from GitHub at
  <https://github.com/poissonconsulting/jaggernaut>. As it uses a number of GitHub packages you should follow the installation instructions on the README file.")
```
...
ToothGrowth <- dplyr::filter(datasets::ToothGrowth, len < 30)
ToothGrowth <- dplyr::mutate(ToothGrowth, len2 = len * 2)
ToothGrowth <- dplyr::select(ToothGrowth, len2, supp, dose)
ToothGrowth <- dplyr::arrange(ToothGrowth, dose, supp, len2)
```
The code fragment `dplyr::` indicates that the following object must be taken from the dplyr package. This can be useful for ensuring R uses the correct object if multiple objects with the same name exist in different packages. Later I use it to remind the reader that a particular package must be loaded to run the code.
...
its name with paired brackets that include any arguments you wish to pass to the function, i.e., `farenheit_to_kelvin(c(0, 32))`.
...
`windows()` function, where by default the width and height are both 7 inches. If working on a Mac using the `quartz()` function instead.
...
is `r round(coefficients(mod)[1],3)` and the slope
is `r round(coefficients(mod)[2],3)`.
