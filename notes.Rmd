...
search()
```
Note `.GlobalEnv` is the workspace. For a detailed description of how R searches and finds objects see http://obeautifulcode.com/R/How-R-Searches-And-Finds-Stuff/.
...
You can install a package from the CRAN site onto your computer by clicking Install on the Packages window and typing its name.
```{r, echo = FALSE, results = 'asis'}
...
The following line of code saves the ToothGrowth data frame to the working directory as 
a csv file called `ToothGrowthMissing`.
```
write.csv(ToothGrowth, "ToothGrowthMissing.csv", row.names = FALSE)
```
